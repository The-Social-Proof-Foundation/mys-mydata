...
/// Wrapper for Mys test cluster with some Seal specific functionality.
...
impl SealTestCluster {
    ...
    pub async fn new(servers: usize, users: usize) -> Self {
        ...
        let servers = (0..servers)
            ...
            .map(|(master_key, public_key)| SealKeyServer {
                server: Server {
                    mys_client: cluster.mys_client().clone(),
    ...
    pub async fn publish(&mut self, module: &str) -> (ObjectID, ObjectID) {
        ...
        // Publish package
        let builder = self.cluster.mys_client().transaction_builder();
        let tx = builder
            .publish(
    ...
    pub async fn register_key_server(
        ...
        let tx = self
            ...
            .move_call(
                ...
                vec![],
                vec![
                    MysJsonValue::from_str(description).unwrap(),
                    MysJsonValue::from_str(url).unwrap(), // Dummy url, not used in this test
                    MysJsonValue::from_str(&0u8.to_string()).unwrap(), // Fix to BF-IBE
                    MysJsonValue::new(json!(pk.to_byte_array().to_vec())).unwrap(),
                ],
                None,
                50_000_000,
                None,
    ...
    pub async fn get_public_keys(&self, object_ids: &[ObjectID]) -> Vec<ibe::PublicKey> {
        ...
        let objects = self
            ...
            .mys_client()
            .read_api()
            .multi_get_object_with_options(object_ids, MysObjectDataOptions::full_content())
            .await
            .unwrap();
        objects
            .into_iter()
