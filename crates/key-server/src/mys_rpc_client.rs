...
use mys_sdk::{
    error::MysRpcResult,
    rpc_types::{
        Checkpoint, CheckpointId, DryRunTransactionBlockResponse, MysObjectDataOptions,
        MysObjectResponse,
    },
    MysClient,
};
use mys_types::base_types::ObjectID;
use mys_types::{dynamic_field::DynamicFieldName, transaction::TransactionData};
...
impl RetriableError for mys_sdk::error::Error {
    fn is_retriable_error(&self) -> bool {
        match self {
            // Low level networking errors are retriable.
            // TODO: Add more retriable errors here
            mys_sdk::error::Error::RpcError(rpc_error) => {
                matches!(
                    rpc_error,
...
async fn mys_rpc_with_retries<T, E, F, Fut>(
...
/// Client for interacting with the MySocial RPC API.
#[derive(Clone)]
pub struct MysRpcClient {
...
impl MysRpcClient {
    ...
    /// Returns a reference to the underlying MysClient.
    ...
    pub async fn get_latest_checkpoint_sequence_number(&self) -> MysRpcResult<u64> {
        mys_rpc_with_retries(
            &self.rpc_retry_config,
    ...
    pub async fn get_checkpoint(&self, checkpoint_id: CheckpointId) -> MysRpcResult<Checkpoint> {
        mys_rpc_with_retries(
            &self.rpc_retry_config,
            "get_checkpoint",
            self.metrics.clone(),
    ...
    pub async fn get_reference_gas_price(&self) -> MysRpcResult<u64> {
        mys_rpc_with_retries(
            ...
            self.metrics.clone(),
            || async { self.mys_client.read_api().get_reference_gas_price().await },
        )
        .await
    }
...
mod tests {
    ...
    use crate::mys_rpc_client::mys_rpc_with_retries;
    use crate::mys_rpc_client::RetriableError;
    ...
    #[tokio::test]
    async fn test_mys_rpc_with_retries_success_first_attempt() {
        ...
        let result = mys_rpc_with_retries(&retry_config, "mock_function", None, || async {
    ...
    #[tokio::test]
    async fn test_mys_rpc_with_retries_success_after_retriable_failures() {
        let retry_config = RetryConfig {
            max_retries: 3,
            min_delay: Duration::from_millis(10),
            max_delay: Duration::from_millis(100),
    ...
    #[tokio::test]
    async fn test_mys_rpc_with_retries_exhausts_all_retries() {
        let retry_config = RetryConfig {
            max_retries: 3,
            min_delay: Duration::from_millis(10),
            max_delay: Duration::from_millis(100),
    ...
    #[tokio::test]
    async fn test_mys_rpc_with_retries_non_retriable_error() {
        let retry_config = RetryConfig {
            max_retries: 3,
            min_delay: Duration::from_millis(10),
            max_delay: Duration::from_millis(100),
    ...
    #[tokio::test]
    async fn test_mys_rpc_with_retries_zero_retries() {
        let retry_config = RetryConfig {
