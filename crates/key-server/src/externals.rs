...
use mys_sdk::rpc_types::CheckpointId;
...
pub(crate) async fn get_latest_checkpoint_timestamp(client: MysClient) -> MysRpcResult<Timestamp> {
...
pub(crate) async fn get_reference_gas_price(client: MysClient) -> MysRpcResult<u64> {
    let rgp = client
        .read_api()
